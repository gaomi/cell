<template>
  <div class="goods-body first-foods">
    <div class="left-goods" ref="menuWrapper">
      <ul>
        <li v-for="(item,index) in foodlist" class="li-meun" :key="index" :class="currentIndex === index ? 'current' : ''">
           <span class="text">
...
<script>
  ...
  export default {
    ...
    ,
    created() {
      this.$https.get('http://localhost:8080/api/goods')
        .then(response => {
